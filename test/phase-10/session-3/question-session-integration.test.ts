...
describe('Session 3: Question Session Integration', () => {
  ...
  describe('question.asked event handling', () => {
    ...
    test('multiple questions can be queued for the same session', () => {
      ...
      useQuestionStore.getState().addQuestion('octob-1', req1)
      useQuestionStore.getState().addQuestion('octob-1', req2)
  ...
  describe('question.replied event handling', () => {
    test('removes question from store by requestID', () => {
      ...
      if (requestId) {
        useQuestionStore.getState().removeQuestion('octob-1', requestId)
      }
      expect(useQuestionStore.getState().getQuestions('octob-1')).toHaveLength(0)
    })
    ...
    test('only removes the specific question, leaving others', () => {
      useQuestionStore
        .getState()
        .addQuestion('octob-1', { id: 'q1', sessionID: 'opc-1', questions: [] })
      useQuestionStore
        .getState()
        .addQuestion('octob-1', { id: 'q2', sessionID: 'opc-1', questions: [] })
  ...
  describe('session cleanup', () => {
    ...
    test('clearSession does not affect other sessions', () => {
      ...
      useQuestionStore
        .getState()
        .addQuestion('octob-2', { id: 'q2', sessionID: 'opc-2', questions: [] })
